...
| **M3 — Durability & recovery** | Event-log replay (RecoveryManager v2); timer reconstruction; backfill + verifier; dual-read of transcript/pending-reply; `PERSISTENCE_MODE=v2` | chaos suite green: worker-kill RTO < 5 s, RPO 0 | 3–4 wk · **IMPLEMENTED 2026-08-13** (v2_events + v2_idempotency on Postgres 16, `rehydrate`/`recoverAll` in [02](../../backend/src/v2/recovery.ts), verifier job, per-call advisory-lock seq; 246 tests green incl. RPO 0/RTO suites — RTO measured ~50 ms for 300+ events) |
